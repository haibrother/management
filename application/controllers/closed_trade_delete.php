 ...
 * 平仓交易编码表日志
 */
class Closed_trade_delete extends CI_Controller
{
	...
	public function trade_list(){
        ...
        //获取当前栏目的所有版本和默认版本
        $data['version'] = $this->trade_delete_model->get_all_version(array('trade_type'=>'closed','version_month'=>$trade_search_data['search_month']));
		...
		$data['item'] = 'closed_trade_delete';
		$data['item_id_field'] = 'user_id';
        $data['excel_export'] = 'closed_trade_delete';
        //多类型行显示
        $data['dedicacated'] = 1; //标识特殊的分页
        $data['page_per_max'] = $page_per_max;
        $data['data_search'] = true;
		$data['data_search_url'] = 'closed_trade_delete/trade_list/';
