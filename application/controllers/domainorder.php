...
class Domainorder extends CI_Controller
{
	...
	function __construct()
	{
		...
		$this->load->model('domainorder_model');
		$this->load->model('server_model');
		$this->load->model('email_model');
		$this->load->model('domain_model');
        if(!$this->permission_model->check_visit_permission(TABLE_DOMAINORDERS, $this->session->userdata('user_group')))
            show_404();
	...
	public function order_list(){
		//配置分页类
		$config['base_url'] = 'domainorder/order_list/';
		$config['per_page'] = PAGE_PER_MAX;
		$config['uri_segment'] = 3;
		$data['page_total'] = $config['total_rows'] = $this->domainorder_model->get_order_form('','','','','','payment');
		...
		//获取货币单位
		$server_result = $this->domain_model->get_domain_to_order('false');
		$server_value = $this->domain_model->option_domain_to_order($server_result,DEFAULT_SERVER_ID);

		//获取支付审核信息数组
		$pay_unit_value = $this->domainorder_model->option_unit_to_order(DEFAULT_UNIT);
		...
		//获取列表
		$result = $this->domainorder_model->get_order_form('', '', PAGE_PER_MAX, intval($this->uri->segment(3)),'','payment');
		...
		foreach ($result as $key => $value){
			...
			foreach($value as $key2 => $value2){
				if($key2 == 'order_domain_id'){
					$server_info = $this->domain_model->get_id_domain($value2);
                    
					$data['result']->$key->$key2 = $this->domain_model->order_domain_info($server_info, false, true);
                  // var_dump($server_info,$data['result']->$key->$key2);exit;
                    ...
                    if($this->session->userdata('user_group') < USER && $value2 == PAY_STATUS_NO){
						$pay_status .= '<br />[<a href="domainorder/pay_order_yes/' . $data['result']->$key->order_id . '" onClick="operate_affirm(\'domainorder\/pay_order_yes\/' . $data['result']->$key->order_id . '\/\');return false;" title="确认已付款！">确认</a>]';
					}elseif($value2 == PAY_STATUS_WAITRENEW){
						$pay_status .= '<br />[<a href="domainorder/reneworder/' . $data['result']->$key->order_id . '" title="域名续费！">续费</a>]';
					}

                    if($value2 == PAY_STATUS_NO && $data['result']->$key->order_user_id == $this->session->userdata('user_login')){
						$pay_status .= '[<a href="domainorder/order_email_remind/' . $data['result']->$key->order_id . '" onClick="operate_affirm(\'domainorder\/order_email_remind\/' . $data['result']->$key->order_id . '\/\');return false;" title="发送email提醒管理员！">email!</a>]';
					}

					$data['result']->$key->$key2 = $pay_status;

				}elseif($key2 == 'order_examine_status'){
					$data['result']->$key->$key2 = '<span class="order-examine-status order-examine-' . $value2 . '"> ' . $EXAMINE_STATUS[$value2] . ' </span>' . ($this->session->userdata('user_group') < USER ? ($value2 == EXAMINE_STATUS_WAIT ? ('<br />[<a href="domainorder/examine_order/' . $data['result']->$key->order_id . '/' . EXAMINE_STATUS_YES . '" onClick="operate_affirm(\'domainorder\/examine_order\/' . $data['result']->$key->order_id . '\/' . EXAMINE_STATUS_YES . '\/\');return false;" title="审核通过！">通过</a>]<br />[<a href="order/examine_order/' . $data['result']->$key->order_id . '/' . EXAMINE_STATUS_NO . '" onClick="operate_affirm(\'domainorder\/examine_order\/' . $data['result']->$key->order_id . '\/' . EXAMINE_STATUS_NO . '\/\');return false;" title="审核不通过！">未通过</a>]') : '') : '');
		...
		$data['data_create'] = array(
								'domain_id' => array(
									...
									'type' => '',
									'name' => '域名',
									'value' => $server_value['domain_name'],
									'maxlength' => '',
									'need' => true,
									'note' => ' 如无您所需要的域名，请先进入 <a href="domain">域名管理页面</a> 添加域名！'),
		...
		$data['data_create_url'] = 'domainorder/create';

		$data['top_item'] = 'domainorder';
		$data['item'] = 'domainorder';
		$data['item_id_field'] = 'order_id';
		$data['edit_able'] = $this->permission_model->check_sql_permission(TABLE_DOMAINORDERS, SQL_ACTION_UPDATE, $this->session->userdata('user_group'));
		$data['data_edit_url'] = 'domainorder/edit/';

		$data['delete_able'] = $this->permission_model->check_sql_permission(TABLE_DOMAINORDERS, SQL_ACTION_DELETE, $this->session->userdata('user_group'));
		$data['data_delete_url'] = 'domainorder/delete_order_single/';
	...
	public function create($renew_old_id = 0){
	   ...
	   //权限检查
		...
		if(empty($_POST['server_id']) || empty($_POST['alipay_id']) || empty($_POST['bank_id']) || empty($_POST['bank_name']) || empty($_POST['pay_amount']) || empty($_POST['due_time'])){
			...
			$order_data = array();
			$order_data['order_domain_id'] = $this->input->post('server_id');
			...
			$check_repeat_result = $this->domainorder_model->get_order_form($order_data);
			if($check_repeat_result != 0){
				echo 2;
			}else{
				$result = $this->domainorder_model->create($order_data, (is_numeric($renew_old_id) && $renew_old_id != 0) ? true : false);
				if(is_numeric($renew_old_id) && $renew_old_id != 0){
					$old_order_data = array();
					$old_order_data['order_pay_status'] = PAY_STATUS_RENEWED;

					$old_order_result = $this->domainorder_model->update_order($renew_old_id, $old_order_data);
	...
	public function delete_order_single($order_id){
		...
		if(!is_numeric($order_id) || !$this->permission_model->check_sql_permission(TABLE_DOMAINORDERS, SQL_ACTION_DELETE, $this->session->userdata('user_group'))){
			echo 'false';
		}else{
			$order_data = array('order_id' => $order_id);
			$result = $this->domainorder_model->delete_order($order_data);
			echo $result;
		}
	}
	...
	public function edit($order_id){
	   //权限检查
		...
		if(!is_numeric($order_id)){
            show_404();
		}elseif(empty($_POST['domain_id']) || empty($_POST['pay_amount']) || empty($_POST['due_time'])){
			$order_data = array('order_id' => $order_id);

			$result = $this->domainorder_model->get_order_form($order_data, '', 1, 0);
			//获取server数据
			$server_result = $this->domain_model->get_domain_to_order();

			foreach ($result as $row){$data['result'] = $row;}

			//获取货币单位
			$pay_unit_value = $this->domainorder_model->option_unit_to_order($data['result']->order_unit);

			//获取货币单位
			$paystatus_value = $this->domainorder_model->option_paystatus_to_order($data['result']->order_pay_status);
			$examinestatus_value = $this->domainorder_model->option_examinestatus_to_order($data['result']->order_examine_status);

			$server_value = $this->domain_model->option_domain_to_order($server_result,$data['result']->order_domain_id);
			...
			$data['data_edit_url'] = 'domainorder/edit/' . $order_id;
			$data['data_edit_backurl'] = 'domainorder';
			...
			if($this->user_model->get_login_user($order_data['order_user_id']) == false){
				...
				$order_data['order_domain_id'] = $this->input->post('domain_id');
				$order_data['order_unit'] = $this->input->post('pay_unit');
    			$order_data['order_alipay_id'] = $this->input->post('alipay_id');
    			$order_data['order_bank_id'] = $this->input->post('bank_id');
    			$order_data['order_bank_name'] = $this->input->post('bank_name');
				...
				$result = $this->domainorder_model->update_order($order_id, $order_data);

				if($order_data['order_pay_status'] == PAY_STATUS_YES){
					$server_data = array();
					$server_data['server_due_time'] = $order_data['order_due_time'];

					$server_result = $this->domain_model->update_domain($order_data['order_domain_id'], $server_data);
				}
	...
	public function pay_order_yes($order_id){
	   //权限检查
		...
		if(!is_numeric($order_id)){
			...
			if($this->domainorder_model->check_order_examine($order_id) == false){
				echo '7';
			}else{
				$order_data = array();
                ...
                if($result == 1){
    				$order_info = $this->domainorder_model->get_order_duetime($order_id);
    				$server_data = array();
    				$server_data['server_due_time'] = $order_info['order_due_time'];

    				$server_result = $this->domain_model->update_domain($order_info['order_domain_id'], $server_data);
                    ...
                    if(strtotime($order_info['order_due_time']) < strtotime (date('Y-m-d 00:00:00', strtotime ("+8 day"))))
                        $this->domainorder_model->pay_order_change($order_id, $order_info['order_user_id'], PAY_STATUS_WAITRENEW);
	...
	public function get_server_info($server_id){
		if(!is_numeric($server_id)){
			...
			$server_info = $this->domain_model->get_id_domain($server_id);
			$result = $this->domain_model->order_domain_info($server_info);
			echo $result;
		}
	}
    ...
    public function order_search(){
        ...
        if(!empty($_POST['submit'])){
            ...
            $order_search_where = $this->domainorder_model->order_transition_search_where($order_search_data['order_search_select'], $order_search_data['order_search_input'], $order_search_data['order_search_ip_select']);
            ...
            $order_search_where = $this->domainorder_model->order_transition_search_where($this->session->userdata('order_search_select'), $this->session->userdata('order_search_input'), $this->session->userdata('order_search_ip_select'));
        }

		//配置分页类
		$config['base_url'] = 'domainorder/order_search/';
		$config['per_page'] = PAGE_PER_MAX;
		$config['uri_segment'] = 3;
        //var_dump($order_search_where) ;
		$data['page_total'] = $config['total_rows'] = $this->domainorder_model->get_order_form($order_search_where, '');
		$config['cur_tag_open'] = '<a class="current">';
		$config['cur_tag_close'] = '</a>';
		$config['first_link'] = '&laquo;';
		$config['last_link'] = '&raquo;';
		...
		//获取server数据
		$server_result = $this->domain_model->get_domain_to_order();
		$server_value = $this->domain_model->option_domain_to_order($server_result,$this->session->userdata('order_search_ip_select'));
        ...
        $data['search_server_ip'] = $server_value['domain_name'];
        ...
        if($data['search_where']){
    		...
    		$result = $this->domainorder_model->get_order_form($order_search_where, '', PAGE_PER_MAX, intval($this->uri->segment(3)));
        	...
        	foreach ($result as $key => $value){
        		foreach($value as $key2 => $value2){
        			if($key2 == 'order_domain_id'){
					   ...
					   $pay_status =  '<span class="order-pay-status order-pay-' . $value2 . '">' . $PAY_STATUS[$value2] . '</span>';
	                   if($this->session->userdata('user_group') < USER && $value2 == PAY_STATUS_NO){
						  $pay_status .= '<br />[<a href="order/pay_order_yes/' . $data['result']->$key->order_id . '" onClick="operate_affirm(\'domainorder\/pay_order_yes\/' . $data['result']->$key->order_id . '\/\');return false;" title="确认已付款！">确认</a>]';
					   }elseif($value2 == PAY_STATUS_WAITRENEW){
						  $pay_status .= '<br />[<a href="domainorder/reneworder/' . $data['result']->$key->order_id . '" title="域名续费！">续费</a>]';
					   }

                       if($value2 == PAY_STATUS_NO && $data['result']->$key->order_user_id == $this->session->userdata('user_login')){
						$pay_status .= '[<a href="order/order_email_remind/' . $data['result']->$key->order_id . '" onClick="operate_affirm(\'domainorder\/order_email_remind\/' . $data['result']->$key->order_id . '\/\');return false;" title="发送email提醒管理员！">email!</a>]';
                        }

					   $data['result']->$key->$key2 = $pay_status;

				    }elseif($key2 == 'order_examine_status'){
						$data['result']->$key->$key2 = '<span class="order-examine-status order-examine-' . $value2 . '"> ' . $EXAMINE_STATUS[$value2] . ' </span>' . ($this->session->userdata('user_group') < USER ? ($value2 == EXAMINE_STATUS_WAIT ? ('<br />[<a href="domainorder/examine_order/' . $data['result']->$key->order_id . '/' . EXAMINE_STATUS_YES . '" onClick="operate_affirm(\'domainorder\/examine_order\/' . $data['result']->$key->order_id . '\/' . EXAMINE_STATUS_YES . '\/\');return false;" title="审核通过！">通过</a>]<br />[<a href="domainorder/examine_order/' . $data['result']->$key->order_id . '/' . EXAMINE_STATUS_NO . '" onClick="operate_affirm(\'domainorder\/examine_order\/' . $data['result']->$key->order_id . '\/' . EXAMINE_STATUS_NO . '\/\');return false;" title="审核不通过！">未通过</a>]') : '') : '');
					}elseif($key2 == 'order_pay_amount'){
						$data['result']->$key->$key2 = $ORDER_PAY_UNIT[$value->order_unit]['display'] . ' ' . $value2;
					}else{
        				if($key2 == 'order_alipay_id' && (int)$value2 != -1)
		...
		$data['top_item'] = 'domainorder';
		$data['item'] = 'domainorder_search';
		$data['item_id_field'] = 'order_id';
		//搜索功能
		$data['data_search'] = true;
		$data['data_search_url'] = 'domainorder/order_search/';
	...
	public function multiorder(){
		...
		$data['top_item'] = 'domainorder';
		$data['item'] = 'domainmultiorder';

		$this->load->view('domainmultiorder', $data);
	}
	...
	public function multicreate(){
	   //权限检查
        ...
        for($i = 0; $i < 5; $i ++){
			...
			$order_data = array();
			$order_data['order_domain_id'] = trim($this->input->post('domain_id'.$i));
        ...
        $result .= '表单 ' . ($i+1) . ' : ';
			...
			if(empty($_POST['due_time'.$i])){
				...
				$check_repeat_result = $this->domain_model->order_domain_info($order_data);
				if($check_repeat_result != 0){
					$result .= '存在重复申请单！';
				}else{
					$query_result = $this->domainorder_model->create($order_data);
	...
	public function excel_export($all_page = 'page', $condition = true){
		...
		$query = $this->db->get(TABLE_DOMAINORDERS, $limit, $now_page);

		$FIELD_DISPLAY = $this->config->item('ORDER_FIELD_DISPLAY');

		$result_array = $this->domainorder_model->excel_sql_result_convert($query, $FIELD_DISPLAY);
	...
	public function order_needrenew(){
		...
		//配置分页类
		$config['base_url'] = 'domainorder/order_needrenew/';
		$config['per_page'] = $page_per_max;
		$config['uri_segment'] = 3;
		$data['page_total'] = $config['total_rows'] = $this->domainorder_model->get_order_form($order_search_where,'','','','','renew');
		$config['cur_tag_open'] = '<a class="current">';
		$config['cur_tag_close'] = '</a>';
		$config['first_link'] = '&laquo;';
		$config['last_link'] = '&raquo;';
		...
		//获取列表
		$result = $this->domainorder_model->get_order_form($order_search_where, '', $page_per_max, intval($this->uri->segment(3)),'','renew');
		...
		$data['top_item'] = 'domainorder';
		$data['item'] = 'domainorder_needrenew';
		$data['item_id_field'] = 'order_id';
		$data['edit_able'] = $this->permission_model->check_sql_permission(TABLE_DOMAINORDERS, SQL_ACTION_UPDATE, $this->session->userdata('user_group'));
		$data['data_edit_url'] = 'domainorder/edit/';
        $data['dedicacated'] = 1; //标识特殊的分页
        $data['page_per_max'] = $page_per_max;
        $data['expiration_time_url'] = 'domainorder/expiration_time';
     ...
     * */
     function expiration_time(){
        ...
        $result = $this->domainorder_model->update_expiration_time($order_id, $order_data);
        echo $result;
     }
}
