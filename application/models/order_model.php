...
class Order_model extends CI_Model
{
	...
	public function create($order_data = array(), $renew = false){
	   //权限检查
		...
		if(!isset($ud['order_server_id']) || !isset($ud['order_alipay_id']) || !isset($ud['order_bank_id']) || !isset($ud['order_bank_name']) || !isset($ud['order_unit'])  || !isset($ud['order_pay_amount']) || !isset($ud['order_due_time']) || !isset($ud['order_remark']))
			return false;

		$data = array(
			'order_server_id' => $ud['order_server_id'],
        ...
        if($result == 1){
            ...
            $this->operationlog_model->write_operationlog('', $new_operationlog_date, TABLE_ORDERS, SQL_ACTION_INSERT, $data, 3);
	...
	function get_order_form($wherearr = '', $order = '', $num = '', $offset = '', $like = '',$type='')
	{
	   //权限检查
        ...
        if(!$this->permission_model->check_sql_permission(TABLE_ORDERS, SQL_ACTION_SELECT, $this->session->userdata('user_group')))
            return false;
		if(!empty($wherearr))
			$this->db->where($wherearr);
    
    //409提及的需求,普通用户不限制查看 edit 417
	...
	{
	   //鏉冮檺妫€鏌?
        if(!$this->permission_model->check_sql_permission(TABLE_ORDERS, SQL_ACTION_DELETE, $this->session->userdata('user_group')))
            return false;
		...
		if(!empty($wherearr)){
            ...
            $this->db->where($wherearr);
			$this->db->update(TABLE_ORDERS, array('order_status' => 'close'));
            $result = $this->db->affected_rows();

            $this->operationlog_model->write_operationlog($old_operationlog_date, '', TABLE_ORDERS, SQL_ACTION_DELETE, '', 3);
	...
	function update_order($order_id = '', $update_data = array())
	{
	   //权限检查
		...
		$this->db->where($wherearr);
		$this->db->update(TABLE_ORDERS, $update_data);
        ...
        switch($search_select){
            ...
            case 4:
                if(empty($search_ip_select))
                    return false;
                return array('order_server_id' => $search_ip_select);
                break;
	...
	function get_order_duetime($order_id = 0){
		...
		$query = $this->db->get(TABLE_ORDERS, 1);
		...
		$result['order_server_id'] = $row->order_server_id;
	...
	public function check_server_order($server_id){
		if(!is_numeric($server_id) || $server_id <= 0)
			return false;

		$this->db->where('order_server_id', $server_id);
		$this->db->where('order_status', 'open');
		$query = $this->db->get(TABLE_ORDERS, 1);
