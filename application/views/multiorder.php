		...
		<div class="content-box"><!-- Start Content Box -->
			...
			<div class="content-box-content">
				...
				<?php else : ?>
					...
					<form id="form-create" action="order/multicreate" method="post" onSubmit="return formCreate();">
						<fieldset>
							...
							<p class="multiorder-p">
								...
								<?php if($key == 'server_id'){ ?><br /><span id="server_id<?php echo $i; ?>_dateinfo" class="server_id_dateinfo"></span><?php } ?>
							</p>
...
function formCreate(){
	...
	for(i=0;i<5;i++){
		server_id = jQuery.trim(jQuery("#form-create select[name='server_id"+i+"']").val());
		...
		if(!((alipay_id == '' || bank_id == '' && bank_name == '') && pay_amount == '0' && due_time == '' && remark == '')){
			...
			if(server_id == '')
				input_need_error_row += '<strong>服务器IP</strong>' + ' 未填写!<br />' ;
			if(alipay_id == '')
...
function () {
	...
	$("#form-date-"+rel).fadeTo(0, 400, function () { // Links with the class "close" will close parent
		get_server_info($("#server_id"+$(this).attr("rel")));
